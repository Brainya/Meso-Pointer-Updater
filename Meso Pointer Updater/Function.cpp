...
BOOL Function::GetModuleSize(HMODULE hModule, LPVOID* lplpBase, LPDWORD lpdwSize)
{
...
DWORD PFAPI Function::GetPatternCB(char *szPattern)
{
	DWORD cb = 0;
...
BOOL PFAPI Function::GeneratePatternMask(char *szPattern, char *buffer)
{
	bool first_nibble = false;
	for (DWORD i=0; i<strlen(szPattern); i++)
	{
...
BOOL PFAPI Function::GeneratePatternBytes(char *szPattern, LPBYTE buffer)
{
	bool first_nibble = false;
	DWORD cb = 0;
	for (DWORD i=0; i<strlen(szPattern); i++) {
...
VOID PFAPI Function::SearchPattern(PFSEARCH *ppf, LPVOID lpvBase, DWORD dwSize)
{
	ppf->lpvResult = 0;
...
DWORD PFAPI Function::FindPattern(char *szPattern, PFSEARCH *ppf, LPVOID lpvBase, DWORD dwSize)
{
